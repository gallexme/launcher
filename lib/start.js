...
module.exports = function start(_opts, output) {
    ...
    var opts = {};
    try {
        opts = ini.parse(fs.readFileSync('./.screepsrc', {
            encoding: 'utf8'
        }));
    } catch (e) {
        if (e.code == 'ENOENT') {
                ...
                if (message == 'storageLaunched') {
                    ...
                    _startProcess('backend',
                        path.resolve(path.dirname(require.resolve('@screeps/backend')), '../bin/start.js'), {
                            ...
                            MONGO_URL: _.isString(opts.mongo) ? opts.mongo : false,
                            REDIS_IP: _.isString(opts.redis_ip) ? opts.redis_ip : false,
                            REDIS_PORT: _.isString(opts.redis_port) ? opts.redis_port : false,
                        });
